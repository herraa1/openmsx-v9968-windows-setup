param([Parameter(Mandatory=$true)][string]$Runtime,[switch]$Standard,[switch]$Optimized,[switch]$CStream)
...
& "$PSScriptRoot/launch.ps1" -Mode $cfg.mode -Standard:$Standard -Optimized:$Optimized -CStream:$CStream -Runtime $Runtime -TestScript "$PSScriptRoot/test.tcl" -WorkRoot $work
